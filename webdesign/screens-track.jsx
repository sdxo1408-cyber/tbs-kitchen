// Order tracking screen — live status with map & rider
function TrackingScreen({ goto, cart, resetCart }) {
  const total = cart.reduce((s, i) => s + i.price * i.qty, 0);
  const stages = [
    { id: 'placed', label: 'Order placed', sub: 'Confirmed at the kitchen', t: 0 },
    { id: 'cooking', label: 'In the kitchen', sub: 'Chef is plating now', t: 8 },
    { id: 'pickup', label: 'Out for delivery', sub: 'Rider has your bag', t: 18 },
    { id: 'delivered', label: 'Delivered', sub: 'Bon appétit', t: 30 },
  ];

  // Simulate progress
  const [stage, setStage] = React.useState(0);
  const [eta, setEta] = React.useState(28);
  ...
  return (
    <div data-scroll-root style={{
      ...
      <div style={{
        ...
        }}>order #TBS-{Math.floor(Math.random() * 9000 + 1000)}</div>
        <h1 style={{
        ...
        <p style={{
          margin: '0 0 22px', fontSize: 13.5, color: BRAND.inkSoft, letterSpacing: '-0.02em',
        }}>{stages[stage].sub}</p>
            ...
            return (
              <div key={s.id} style={{
                ...
                <div style={{ flex: 1, paddingTop: 1 }}>
                  <div style={{
                    fontSize: 14, color: BRAND.ink, letterSpacing: '-0.02em',
        ...
        {/* Rider card (appears at pickup stage) */}
        {stage >= 2 && (
          <div style={{
        ...
        {/* Order summary */}
        <div style={{
          background: BRAND.blush, borderRadius: 20, padding: 18,
        }}>
          <div style={{
            fontFamily: 'ui-monospace, monospace', fontSize: 10,
            letterSpacing: '0.18em', textTransform: 'uppercase',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {cart.length === 0 ? (
              <div style={{ fontSize: 13, color: BRAND.inkSoft }}>
                <em style={{ fontFamily: 'Instrument Serif, serif' }}>1× Classic Buddha Bowl</em>, <em style={{ fontFamily: 'Instrument Serif, serif' }}>1× Anabolic Chicken Meal</em>
              </div>
            ) : (
              cart.map(i => (
...
// SVG faux-map with animated rider path
function FakeMap({ stage }) {
  // 0..3 stage; position rider along path
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 400 380" preserveAspectRatio="xMidYMid slice"
        ...
        {/* route */}
        <path id="route" d="M70 320 Q120 280 160 240 T260 170 L320 110"
        ...
        {/* rider — interpolated along simplified path */}
        <RiderDot t={t} />
        {/* labels */}
...
function RiderDot({ t }) {
  // approximate route midpoints
  const pts = [
    [70, 320], [120, 280], [160, 240], [210, 200], [260, 170], [320, 110],
  ];
