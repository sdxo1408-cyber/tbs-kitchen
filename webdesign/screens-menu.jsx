// Menu screen with category tabs + filter chips
function MenuScreen({ goto, addToCart, openItem }) {
  ...
  const filtered = MENU.filter(m => {
    ...
    for (const f of filters) {
      ...
      else if (f === 'veg') { /* all our items have veg variants; pass */ }
  ...
  return (
    <div data-scroll-root style={{
        ...
        {FILTERS.map(f => {
          ...
          return (
            <button key={f.id} onClick={() => toggleFilter(f.id)}
              ...
              {Icon[f.icon] && Icon[f.icon](11, on ? '#fff' : BRAND.forest)}
              {f.label}
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'Instrument Serif, serif', fontStyle: 'italic', fontSize: 14 }}>
          {filtered.length}
        </span> {filtered.length === 1 ? 'meal' : 'meals'} ready to order
      </div>
  ...
  return (
    <div
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div style={{
          fontFamily: 'DM Sans, sans-serif', fontSize: 14,
          color: BRAND.forest, letterSpacing: '-0.02em', fontWeight: 500,
        }}>₹{startPrice(m)}</div>
