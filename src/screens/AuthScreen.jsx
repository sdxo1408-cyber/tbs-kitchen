...
export default function AuthScreen({ mode, goto, onAuth }) {
  ...
  return (
    <div data-scroll-root style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', bottom: 40, left: 28, right: 28, color: BRAND.cream }}>
          <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          fontSize: 28, fontWeight: 400, letterSpacing: '-0.04em', color: BRAND.ink,
        }}>{isLogin ? 'Sign in' : 'Create account'}</h2>
        <p style={{ margin: '0 0 24px', fontSize: 13, color: BRAND.inkSoft, letterSpacing: '-0.02em' }}>
          {isLogin ? 'Pick up where you left off.' : 'Better food is one form away.'}
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {!isLogin && <Field label="Full name" value={name} onChange={setName} placeholder="Aanya Sharma" />}
          <Field label="Email" value={email} onChange={setEmail} placeholder="you@kitchen.com" type="email" />
          {!isLogin && <Field label="Phone" value={phone} onChange={setPhone} placeholder="+91 98xxx xxxxx" type="tel" />}
          <Field
        ...
        <div style={{ marginTop: 28, textAlign: 'center', fontSize: 13, color: BRAND.inkSoft, letterSpacing: '-0.02em' }}>
